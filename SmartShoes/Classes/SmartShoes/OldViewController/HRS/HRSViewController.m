...
-(void)updatePlotSpace
{
    ...
    plotSpace.allowsUserInteraction = NO;
    plotSpace.xRange = [CPTPlotRange plotRangeWithLocationDecimal:CPTDecimalFromInt(plotXMinRange)
                                                    lengthDecimal:CPTDecimalFromInt(plotXMaxRange)];
    plotSpace.yRange = [CPTPlotRange plotRangeWithLocationDecimal:CPTDecimalFromInt(plotYMinRange)
                                                    lengthDecimal:CPTDecimalFromInt(plotYMaxRange)];
    
    CPTXYAxisSet *axisSet = (CPTXYAxisSet *)self.graph.axisSet;
    axisSet.xAxis.majorIntervalLength = [NSDecimalNumber decimalNumberWithDecimal:CPTDecimalFromInt(plotXInterval)];
}
...
//    hrPeripheral.delegate = self;
    ...
    [self.delegate scannerData:manager didPeripheralSelected:peripheral];
...
//                [self addHRValueToGraph:[self decodeHRValue:characteristic.value]];
                ...
                if ((bpmValue & (1 << 0)) == 0){
                    ...
                    hrLocation.text = [NSString stringWithFormat:@"%@℃",vv];
                    [hrLocation sizeToFit];
                    if (vv.floatValue > 45) {
                    ...
                    int lastV = value1 % 10;
                    NSString *vv = [NSString stringWithFormat:@"%@.%@",@(value2),@(lastV)];
                    hrValue.text = [NSString stringWithFormat:@"%@%%",vv];
                    [hrValue sizeToFit];
